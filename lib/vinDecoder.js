...
// Rough mapping from a real trim name (e.g. "XLT", "SS", "Limited") onto
// our own coarse base/mid/upper/top multiplier buckets. Priority is
// top > upper > base, so e.g. "Limited SS" lands on top. Anything that
// doesn't match a known badge is left alone (defaults to whatever the
// form already has, normally "mid") rather than guessed.
const TOP_TOKENS = new Set([
  'ss', 'srt', 'z06', 'zl1', 'raptor', 'trackhawk', 'redeye', 'hellcat', 'demon', 'nismo', 'amg',
]);
const TOP_PHRASES = ['type r', 'trd pro', 'gt350', 'gt500', 'm sport'];

const UPPER_TOKENS = new Set([
  'limited', 'platinum', 'denali', 'premier', 'touring', 'titanium', 'summit', 'reserve',
  'elite', 'lariat', 'sel', 'ultimate', 'signature',
]);
const UPPER_PHRASES = ['king ranch', 'high country', 'trd off-road', 'black edition'];

const BASE_TOKENS = new Set(['base', 'tradesman', 'sv', 'sr', 'le', 'ce', 'cx', 'value', 's']);
const BASE_PHRASES = ['work truck'];

function guessTrimFromText(trimText) {
  if (!trimText) return null;
  const text = trimText.toLowerCase().replace(/\s+/g, ' ').trim();
  const tokens = text.split(' ');

  if (TOP_PHRASES.some((p) => text.includes(p)) || tokens.some((t) => TOP_TOKENS.has(t))) {
    return 'top';
  }
  if (UPPER_PHRASES.some((p) => text.includes(p)) || tokens.some((t) => UPPER_TOKENS.has(t))) {
    return 'upper';
  }
  if (BASE_PHRASES.some((p) => text.includes(p)) || tokens.some((t) => BASE_TOKENS.has(t))) {
    return 'base';
  }
  return null;
}
...
export async function decodeVin(vin) {
  ...
  const trimGuess = guessTrimFromText(decodedTrim);
  if (trimGuess) {
    patch.trim = trimGuess;
  }
