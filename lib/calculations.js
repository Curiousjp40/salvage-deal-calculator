 ...
 * and actual mileage vs. an expected-mileage baseline.
 */
export function estimateCleanValue({ segment, year, mileage, trimMultiplier }) {
  ...
  let value = segmentInfo.baseValue * depreciationFactor * multiplier;
...
export function computeDeal({
  ...
  const auctionFee = bid * (auctionFeePct / 100);
  ...
  if (hasCleanValue) {
    const feeTaxMultiplier = (1 + auctionFeePct / 100) * (1 + taxRate / 100);
    const targetCost = cleanValue * (targetPct / 100);
    suggestedMaxBid = Math.max(0, (targetCost - fixedCosts) / feeTaxMultiplier);
  }
